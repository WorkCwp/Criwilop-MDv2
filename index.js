...
global.handleBotError = async (client, m, error, context = "") => {
  try {
    const errorText = `
⚠️ *Ocurrió un error* ⚠️

📍 *Comando:* ${m?.text || "Desconocido"}
💬 *Chat:* ${m?.chat || m?.key?.remoteJid || "Desconocido"}
👤 *Usuario:* ${m?.sender || "Desconocido"}
...
\`\`\`
    ...
    console.error("No se pudo enviar el mensaje de error:", err);
  }
};

  if (!client.authState.creds.registered) {
    const phoneNumber = await question(
      log.warn("Numero del cliente:\n") +
        log.info("Ejemplo: 573238031915\n"),
    );
    try {
      log.info("Cargando Paring code...");
      const pairing = await client.requestPairingCode(phoneNumber, "CRIWILOP");
      log.success(
...
client.findJidByLid = (lid) => {
  try {
    return client.decodeJid(lid)
  } catch {
    return lid
  }
}
